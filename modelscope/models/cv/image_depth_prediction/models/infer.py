...
class InferenceHelper:
    ...
    @torch.no_grad()
    def postprocess(self, pred_depth):
        pred = np.clip(pred_depth.cpu().numpy(), self.min_depth, self.max_depth)

        # Flip
        # image = torch.Tensor(np.array(image.cpu().numpy())[..., ::-1].copy()).to(self.device)

        # pred_lr = pred_depth[-1]
        # pred_lr = np.clip(pred_lr.cpu().numpy()[..., ::-1], self.min_depth, self.max_depth)

        # Take average of original and mirror
        # final = 0.5 * (pred + pred_lr)
        final = pred
        final = nn.functional.interpolate(torch.Tensor(final), [480, 640],
                                          mode='bilinear', align_corners=True).cpu().numpy()
...
#     img = Image.open("/home/gyalex/projects/opensrc/nice-slam/Datasets/Demo/frames/color/0.jpg")
#     start = time()
